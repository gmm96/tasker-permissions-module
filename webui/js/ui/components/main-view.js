const MainView = (() =>
{
    ...
    function renderList(tabApps, appViewModelDict, onAppClick)
    {
        ...
        {
            list.innerHTML =
                '<div class="empty-message">No applications found in this category</div>';
        }
    }
    ...
    function renderMainActionButtons(appViewModels)
    {
        ...
        for (const appVM of appViewModels)
        {
            if (!appVM.disabled)
            {
                if (appVM.status !== AppStatus.NONEGRANTED)
                {
                    revokeAllBtnDisabled = false;
                }

                if (appVM.status !== AppStatus.ALLGRANTED)
                {
                    grantAllBtnDisabled = false;
                }
            }
        }
