...
import * as Shizuku from '../../infrastructure/shizuku.js'; 
import * as Dialogs from '../../ui/components/dialogs.js';
import * as LoadingSpinner from '../../ui/components/spinner.js';
...
export async function inspectAppPermissions(app)
{
...
export async function processPermissions(pkg, perms, action)
{
...
export async function runBulkAction(options, onComplete, onGoBack)
{
    const {
        action,
        targets,
...
export function runSelectedAction(action, activeApp, selectedPerms, onComplete, onGoBack)
{
    if (!activeApp) return;
